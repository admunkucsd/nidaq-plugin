...
EditorBackground::EditorBackground (int nAI, int nDI) : nAI (nAI),
                                                        nDI (nDI) {}
...
FifoMonitor::FifoMonitor (NIDAQThread* thread_) : thread (thread_),
                                                  fillPercentage (0.0)
{
...
AIButton::AIButton (int id_, NIDAQThread* thread_) : id (id_),
                                                     thread (thread_),
                                                     enabled (true)
{
    startTimer (500);
...
DIButton::DIButton (int id_, NIDAQThread* thread_) : id (id_),
                                                     thread (thread_),
                                                     enabled (true)
{
    startTimer (500);
...
SourceTypeButton::SourceTypeButton (int id_, NIDAQThread* thread_, SOURCE_TYPE source) : id (id_),
                                                                                         thread (thread_)
{
    update (source);
}
...
void NIDAQEditor::draw()
{
    ...
    configureDeviceButton->setFont (FontOptions ("Small Text", 12, Font::plain));
